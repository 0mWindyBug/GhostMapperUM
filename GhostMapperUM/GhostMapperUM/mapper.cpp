...
bool MapDriver(HANDLE IntelDrvHandle, BYTE* RawImage)
{
	...
	// Allocate local memory for driver image 
	...
	{
		uint64_t PteAddress = (uint64_t)GetPTEForVA(IntelDrvHandle, CurrentAddress,PteBaseAddress, KernelBase);
	...
	Log(L"[*] called DriverEntry of mapped driver!" << std::endl);
