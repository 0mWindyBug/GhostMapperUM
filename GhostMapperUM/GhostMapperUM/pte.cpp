...
bool AvoidRWXPtes(HANDLE IntelDrvHandle , const std::vector<uint64_t>& ExecutablePtes, const std::vector<uint64_t>& WriteablePtes)
{
	for (uint64_t ExecPte : ExecutablePtes)
	{
		pte PteMemory;
		if (!intel_driver::ReadMemory(IntelDrvHandle, ExecPte, &PteMemory, sizeof(pte)))
		{
			Log(L"[-] failed to read ghost driver pte" << std::endl);
			return false;
		}
		PteMemory.rw = false;
		if (!intel_driver::WriteMemory(IntelDrvHandle, ExecPte, &PteMemory, sizeof(pte)))
		{
			Log(L"[-] failed to patch ghost driver pte" << std::endl);
			return false;
		}
	}
	for (uint64_t WritePte : WriteablePtes)
	{
		pte PteMemory;
		if (!intel_driver::ReadMemory(IntelDrvHandle, WritePte, &PteMemory, sizeof(pte)))
		{
			Log(L"[-] failed to read ghost driver pte" << std::endl);
			return false;
		}
		PteMemory.nx = true;
		if (!intel_driver::WriteMemory(IntelDrvHandle, WritePte, &PteMemory, sizeof(pte)))
		{
			Log(L"[-] failed to patch ghost driver pte" << std::endl);
			return false;
		}
