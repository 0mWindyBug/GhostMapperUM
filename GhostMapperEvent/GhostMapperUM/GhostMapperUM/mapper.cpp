...
bool MapDriver(HANDLE IntelDrvHandle, BYTE* RawImage)
{
	...
	// create shared event to sync restoring of memory 
	if (RESTORE)
	{
		hEvent = CreateEventW(NULL, FALSE, FALSE, RESTORE_EVENT);
		if (hEvent == INVALID_HANDLE_VALUE || hEvent == NULL)
			return false;
	}
	AutoHandle AutoEvent(hEvent);
	...
	// wait for driver to finish executing and restore original driver if config says so 
	if (RESTORE)
	{
		Log(L"[*] waiting for driver to finish executing..." << std::endl);
		DWORD WaitStatus = WaitForSingleObject(hEvent, INFINITE);
		if (WaitStatus == WAIT_OBJECT_0)
		{
			Log(L"[*] driver has finished executing , cleaning up" << std::endl);
			RestoreOriginalDriver(IntelDrvHandle, GhostDriverBase, OriginalMemory, ImageSize, std::move(OriginalPtes), PteBaseAddress);
		}
		else
			Log(L"[*] failed waiting on restore event ( Status : 0x" << std::hex << GetLastError() << L")" << std::endl);
	}
